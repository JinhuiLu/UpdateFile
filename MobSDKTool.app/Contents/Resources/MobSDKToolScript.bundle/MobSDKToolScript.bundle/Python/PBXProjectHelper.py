...
class PBXProjectHelper (object) :
	...
	def __parseDocument (self, projData) :
		...
		# 过滤空白字符
		# (projData, num) = re.subn ("\n\s*\r", "", projData)
		# (projData, num) = re.subn("(?!\w)\s+\W", "", projData)
		# (projData, num) = re.subn("\W\s+\w", "", projData)
...
class XCBuildConfiguration(PBXObject):
	...
	def setBitCode(self, value):
		...
		self.setBuildSetting("ENABLE_BITCODE", value)

	def removeFrameworkSearchPath (self, path) :
		if path == None :
			return
		searchPaths = self.getBuildSetting("FRAMEWORK_SEARCH_PATHS")
		if searchPaths == None :
...
class PBXGroup (PBXNavigatorItem):
	...
	def addGroup(self, name, path = None, sourceTree = PBXSourceTree.GROUP):
		...
		nameStr = self.helper.converToString(name)
		if self.check_contain_chinese(nameStr):
			nameStr = '\"' + nameStr + '\"'


		groupInfo ["name"] = nameStr
	...
	def addSystemFramework (self, frameworkName, target) :
		return self.addFramework('System/Library/Frameworks/' + frameworkName, target, PBXSourceTree.SDK_ROOT)

	# 添加框架
	# @param frameworkPath 框架路径
	# @param target 项目Target对象
	# @param sourceTree 源位置
	def addFramework (self, frameworkPath, target, sourceTree = PBXSourceTree.GROUP) :
		return self.addFile (frameworkPath, target, sourceTree)

	# 添加系统动态库
	# @param dylibName  动态库名称
	# @param target 项目Target对象
	def addSystemDylib (self, dylibName, target) :
		return self.addDylib ("usr/lib/" + dylibName, target, PBXSourceTree.SDK_ROOT)

	# 添加动态库
	# @parma dylib  动态库路径
	# @param target 项目Target对象
	# @param souceTree 源位置
	def addDylib (self, dylibPath, target, sourceTree = PBXSourceTree.GROUP) :
		return self.addFile (dylibPath, target, sourceTree)

	# 添加静态库
	# @param staticLibPath 静态库路径
	# @param target 项目Target对象
	# @param souceTree 源位置
	def addStaticLib (self, staticLibPath, target, sourceTree = PBXSourceTree.GROUP) :
		return self.addFile (staticLibPath, target, sourceTree)
	...
	# @param sourceTree 源位置
	# @return 文件对象
	def addFile (self, path, target, sourceTree = PBXSourceTree.GROUP) :
...
# python PBXProjectHelper.py /Users/lujh/Desktop/ABC/ABC.xcodeproj/project.pbxproj
# def main():
#
...
#
# 		frameworkGroup = parser.project.mainGroup.find("Frameworks")
...
# 		frameworkGroup.addSystemFramework("AVFoundation.framework", parser.project.targets[0])
#
